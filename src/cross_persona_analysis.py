...
async def batch_run_analysis(
    ...
    """Runs analysis for fixed analyst personas on the given transcript."""

    print("Starting concurrent analysis runs for analyst personas: " + ", ".join(ANALYST_PERSONAS))

    tasks = [
        analyze_strategy_chunks(
            analyst_persona=ap,
            transcript_owner=transcript_owner,
            transcript_key=transcript_key,
            model=model,
        )
        for ap in ANALYST_PERSONAS
    ]
    await asyncio.gather(*tasks)
    print("\nAll analysis runs completed.")


if __name__ == "__main__":
    transcript_owner = sys.argv[1]
    transcript_key = sys.argv[2]
    model = sys.argv[3]
    asyncio.run(batch_run_analysis(transcript_owner, transcript_key, model))
